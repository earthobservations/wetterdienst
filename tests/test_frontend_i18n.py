...
"""Test the frontend translation catalogs against what the backend actually serves.

The frontend labels every canonical parameter and every unit type in eleven languages. Nothing on
the frontend side can notice when a parameter or a unit type is *added here*: its own guard reads
...
`  key: 'label',` shape and the parity test on the frontend side keeps them that way.
"""
...
GLOSSARY_EN = FRONTEND / "i18n" / "glossary" / "en.ts"
LOCALE_EN = FRONTEND / "i18n" / "locales" / "en.json"

pytestmark = pytest.mark.skipif(not FRONTEND.exists(), reason="frontend not part of this checkout")
...
def test_frontend_labels_every_canonical_parameter() -> None:
    """Every parameter the glossary endpoint serves must have a frontend label.
    ...
    languages. Add the name to `frontend/i18n/glossary/*.ts`, in every locale.
    """
...
def test_frontend_names_every_unit_type() -> None:
    """Every unit type the converter knows must have a frontend label.

    The glossary quantity filter and the Explorer unit rows name the quantity, and fall back to the
    raw id the same way. Add the label to `settings.unit*` in `frontend/i18n/locales/*.json`, in
    every locale -- the frontend parity test then holds the other ten to it.
    """
    ...
    assert not missing, f"unit types with no frontend label: {missing}"
