...
from wetterdienst.additionals.functions import (
    ...
    retrieve_period_type_from_filename,
...
def test_retrieve_time_resolution_from_filename():
    ...
    assert (
        retrieve_time_resolution_from_filename("tageswerte__2019.csv")
...
def test_retrieve_parameter_from_filename():
    assert (
        retrieve_parameter_from_filename("bidb_!!_st_.xml", TimeResolution.HOURLY)
        == Parameter.SOLAR
    )
    assert (
        retrieve_parameter_from_filename(
            "10000_historical_nieder_.txt", TimeResolution.MINUTE_1
        )
        == Parameter.PRECIPITATION
    )
    assert (
        retrieve_parameter_from_filename("klima_climate_kl_.csv", TimeResolution.DAILY)
        == Parameter.CLIMATE_SUMMARY
    )
    assert (
        retrieve_parameter_from_filename(
            "klima_climate_kl_.csv", TimeResolution.MINUTE_1
        )
        is None
    )
