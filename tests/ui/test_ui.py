...
@pytest.mark.slow
@pytest.mark.ui
def test_app_layout(wetterdienst_ui, dash_duo):

    # Sanity check if we are on the right page.
    assert dash_duo.find_element("h1").text == "Wetterdienst UI"

    # Roughly verify the application elements.
    assert dash_duo.find_element("#navigation")
    assert dash_duo.find_element("#map")
    assert dash_duo.find_element("#graph")


@pytest.mark.slow
@pytest.mark.ui
def test_app_data_stations(wetterdienst_ui, dash_duo):
    """
    Verify if data for "stations" has been correctly propagated.
    """

    dash_duo.wait_for_element_by_id("hidden-div-metadata", timeout=5)
    time.sleep(0.5)
