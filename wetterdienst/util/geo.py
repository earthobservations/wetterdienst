...
def convert_dms_string_to_dd(dms: pl.Series) -> pl.Series:
    ...
    dms = dms.select(
        pl.col("dms").list.get(0).cast(pl.Float32).alias("degrees"),
        pl.col("dms").list.get(1).cast(pl.Float32).alias("minutes"),
        pl.col("dms").list.get(2).cast(pl.Float32).alias("seconds"),
    )
