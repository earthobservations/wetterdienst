...
REMOTE_META_INDEX_FILE_PATH = (
    ...
    "sws_stations_xls.xlsx?__blob=publicationFile&v=11"
)
...
def create_meta_index_for_road_weather() -> pd.DataFrame:
    """
    ...
    metaindex[Columns.LONGITUDE.value] = (
        metaindex[Columns.LONGITUDE.value].replace(",", ".", regex=True).astype(float)
    )
    metaindex[Columns.LATITUDE.value] = (
        metaindex[Columns.LATITUDE.value].replace(",", ".", regex=True).astype(float)
    )

    metaindex[Columns.FROM_DATE.value] = DEFAULT_FROM_DATE
    metaindex[Columns.TO_DATE.value] = DEFAULT_TO_DATE

    return metaindex.loc[
        metaindex[Columns.HAS_FILE.value].isna(), ROAD_METADATA_COLUMNS
    ]
