...
class DwdRoadWeatherObservationValues:
    ...
    def __init__(
        ...
        self.metaindex = self.metaindex[
            self.metaindex[Columns.STATION_GROUP.value] != "XX"
        ]
    ...
    def _collect_data_by_station_name(self, station_name: str) -> pd.DataFrame:
        ...
        self.metaindex = self.metaindex.loc[
            self.metaindex[Columns.NAME.value] == station_name, :
        ]
    ...
    def _collect_data_by_station_group(
        ...
        if self.period != DwdRoadWeatherObservationPeriod.LATEST:
            raise NotImplementedError(
                "Actually only latest as period type is supported"
            )
        else:
